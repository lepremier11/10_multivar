...
- Dimensionality reduction , a dimension is column 
...
- Based on Y (outcome) existence:
    - Supervised, when you give the y variable 
    - Unsupervised, when you don't give the y variable
...
weather %>%
  ...
  mutate(r2 = map_dbl(data,
                  ~lm(strength_gtex ~ value,
                      data = .x) %>%
                    glance(.) %>% # glance give fit metrics aic, bic, r2, adj rsq
                    pull(r.squared) #just pulling r.sq from the fit metrics
                  )) %>%
  arrange(desc(r2))
...
lm_dayl.apr <- lm(strength_gtex ~ mean_dayl.s_Apr,
                  data = weather) %>%
  tidy() %>% # tidy give you estimates of a linear model
...
lm_dayl.apr %>%
  ...
  ggplot(aes(x=mod)) +
  geom_pointrange(aes (y = estimate, ymin = estimate - std.error,
...
lm(strength_gtex ~ mean_dayl.s_Apr + 
                  mean_dayl.s_May,
   data = weather) %>%
  vif() # if you have more than 3 variables, you will have different vif for each pair of variables
```
...
In those cases, predictors with vastly different scales (e.g. mean_tmin.c_Jan from -10 to 10 C vs mean_dayl.s_Jul from 48000 to 51000) need to be normalized so measurement scale does not affect variable importance.Higher magnitude might drive significance but it is just an artifact.    
...
- Is an unsupervised analysis (no outcome) - do not have a y varible
...
- All PCs are **orthogonal** (i.e., independent of each other). vs variables are not s 
...
Let's plot PC1 vs PC2 scores, look for any groupings.It just multiply the weight of the component by each site_year and add them up

```{r PC1 vs PC2}
fviz_pca_ind(mod_pca, label = F)
```
...
lm_pca <- lm(strength_gtex ~ PC1 + PC2 + PC3 + PC4, data = weather_postpca) 
...
vif(lm_pca)
```
this is why we used PCA
```{r pca regression plots}
# Plotting strength vs PC1
ggplot(weather_postpca, 
       aes(x = PC1, y = strength_gtex))+ geom_point() + geom_smooth(method = "lm")
```
